...
class TestGetErrorLogPath:
    ...
    def test_returns_path_object(self, _log_dir):
        path = LogService.get_error_log_path()
        assert isinstance(path, Path)

    def test_filename_is_errors_jsonl(self, _log_dir):
        path = LogService.get_error_log_path()
...
class TestLogError:
    ...
    def test_entry_default_retries_is_zero(self, _log_dir, log_file):
        LogService.log_error("sync", "error")
        entry = json.loads(log_file.read_text())
        assert entry["retries"] == 0

    def test_entry_with_retries(self, _log_dir, log_file):
        LogService.log_error("sync", "timeout", retries=3)
        entry = json.loads(log_file.read_text())
        assert entry["retries"] == 3

    def test_entry_with_context(self, _log_dir, log_file):
        ctx = {"task_id": "abc-123", "profile": "work"}
        LogService.log_error("complete", "error", context=ctx)
        entry = json.loads(log_file.read_text())
        assert entry["context"] == ctx

    def test_entry_default_context_is_empty_dict(self, _log_dir, log_file):
        LogService.log_error("list", "error")
        entry = json.loads(log_file.read_text())
        assert entry["context"] == {}

    def test_multiple_calls_append_lines(self, _log_dir, log_file):
    ...
    def test_appended_entries_are_independent_json(self, _log_dir, log_file):
...
class TestGetRecentErrors:
    ...
    def test_returns_empty_list_when_no_file(self, _log_dir):
        result = LogService.get_recent_errors()
        assert result == []

    def test_returns_empty_list_for_empty_file(self, _log_dir, log_file):
        log_file.write_text("", encoding="utf-8")
        result = LogService.get_recent_errors()
        assert result == []

    def test_returns_entries_from_file(self, _log_dir, log_file):
        _write_entry(log_file, command="add", error="err1")
        _write_entry(log_file, command="list", error="err2")
        result = LogService.get_recent_errors()
        assert len(result) == 2

    def test_entries_are_dicts(self, _log_dir, log_file):
        _write_entry(log_file)
        result = LogService.get_recent_errors()
        assert all(isinstance(e, dict) for e in result)

    def test_respects_limit(self, _log_dir, log_file):
        for i in range(10):
            _write_entry(log_file, command=f"cmd{i}", error=f"err{i}")
        result = LogService.get_recent_errors(limit=3)
        assert len(result) == 3

    def test_default_limit_is_ten(self, _log_dir, log_file):
        for i in range(15):
            _write_entry(log_file, command=f"cmd{i}", error=f"err{i}")
        result = LogService.get_recent_errors()
        assert len(result) == 10

    def test_returns_most_recent_first(self, _log_dir, log_file):
    ...
    def test_skips_malformed_json_lines(self, _log_dir, log_file):
        log_file.write_text(
            'not-json\n'
    ...
    def test_skips_empty_lines(self, _log_dir, log_file):
    ...
    def test_since_hours_filters_old_entries(self, _log_dir, log_file, _patched_dt):
    ...
    def test_since_hours_includes_boundary_entry(self, _log_dir, log_file, _patched_dt):
    ...
    def test_since_hours_none_returns_all_entries(self, _log_dir, log_file):
        for i in range(5):
...
class TestGetUnreadErrors:
    ...
    def test_returns_empty_list_when_no_file(self, _log_dir, _patched_dt):
        assert LogService.get_unread_errors() == []

    def test_returns_unacknowledged_entries(self, _log_dir, log_file, _patched_dt):
    ...
    def test_excludes_acknowledged_entries(self, _log_dir, log_file, _patched_dt):
    ...
    def test_returns_empty_when_all_acknowledged(self, _log_dir, log_file, _patched_dt):
        _write_entry(log_file, acknowledged=True)
        _write_entry(log_file, acknowledged=True)
        assert LogService.get_unread_errors() == []

    def test_entries_without_acknowledged_key_are_unread(
        self, _log_dir, log_file, _patched_dt
    ):
...
class TestMarkErrorsAsRead:
    ...
    def test_returns_zero_when_no_file(self, _log_dir):
        assert LogService.mark_errors_as_read() == 0

    def test_marks_recent_entries_acknowledged(self, _log_dir, log_file, _patched_dt):
    ...
    def test_old_entries_are_not_marked(self, _log_dir, log_file, _patched_dt):
...
class TestClearOldErrors:
    ...
    def test_removes_entries_older_than_days(self, _log_dir, log_file, _patched_dt):
    ...
    def test_returns_count_of_removed_entries(self, _log_dir, log_file, _patched_dt):
    ...
    def test_custom_days_parameter(self, _log_dir, log_file, _patched_dt):
    ...
    def test_skips_malformed_json_lines_silently(self, _log_dir, log_file, _patched_dt):
