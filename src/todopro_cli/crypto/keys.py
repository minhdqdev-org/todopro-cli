...
from todopro_cli.crypto.exceptions import KeyDerivationError
...
class MasterKey:
    ...
    def __repr__(self) -> str:
        ...
        return f"MasterKey(key_hash={hashlib.sha256(self.key_bytes).hexdigest()[:16]}...)"

    def __eq__(self, other: object) -> bool:
        """Compare keys securely."""
        if not isinstance(other, MasterKey):
            return NotImplemented
        return hashlib.sha256(self.key_bytes).digest() == hashlib.sha256(other.key_bytes).digest()
