...
async def _resolve_project_name(project_input: str, strategy) -> str:
    ...
    # Prefix matches next
    prefix_matches = [
        p for p in all_projects if p.name.lower().startswith(project_input.lower())
    ]
    ...
    if not candidates:
        raise ValueError(
            f"No project matching '{project_input}'. Available: {', '.join(names)}"
        )
    if len(candidates) == 1:
    ...
    try:
        ...
        pass
    raise ValueError("Invalid choice. Please specify an exact project name or ID.")
...
async def edit_command(
    ...
    content: str | None = typer.Option(
        None, "--content", "-c", help="New task content"
    ),
    description: str | None = typer.Option(
        None, "--description", help="New description"
    ),
    due: str | None = typer.Option(
        None, "--due", "-d", help="New due date (e.g. 'tomorrow', '2026-03-01 14:00')"
    ),
    priority: int | None = typer.Option(
        None,
        "--priority",
        "-p",
        help="New priority (1=low, 2=medium, 3=high, 4=urgent)",
    ),
    project: str | None = typer.Option(
        None, "--project", help="New project (name or ID, fuzzy search supported)"
    ),
    ...
    flag_mode = any(
        x is not None for x in [content, description, due, priority, project]
    )
    ...
    if flag_mode:
        ...
        console.print(
            "[dim](Press Enter to keep current value, type a value to change)[/dim]\n"
        )
        ...
        table.add_row(
            "[dim]Priority[/dim]",
            f"{task.priority} ({_PRIORITY_NAMES.get(task.priority, '?')})",
        )
        ...
        raw_project = _prompt_field(
            "Project (name or ID)",
            current_project_display if current_project_display != "(none)" else "",
        )
        ...
        if raw_priority is not None:
            try:
                ...
                format_error(
                    f"Invalid priority '{raw_priority}'. Keeping current value."
                )

    # Nothing to update
    if not any(
        x is not None
        for x in [new_content, new_description, new_due, new_priority, new_project]
    ):
